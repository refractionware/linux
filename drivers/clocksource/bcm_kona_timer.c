...
kona_wait_for_compare_val_sync(void __iomem *base, int ch_num)
{
	...
	if (!loop_limit)
		pr_err("kona-timer: compare value sync timed out for channel %d (%x)\n",
		       ch_num, reg);
}
...
kona_wait_for_compare_enable_sync(void __iomem *base, int ch_num, int target)
{
	...
	if (!loop_limit)
		pr_err("kona-timer: compare enable sync timed out for channel %d\n",
		       ch_num);
}
				  ...
				  struct clock_event_device *evt)
{
	...
	/* Only read low offset */
	lsw = readl_relaxed(timer->base + KONA_GPTIMER_STCLO_OFFSET);

	/* Load the "next" event tick value */
	writel_relaxed(lsw + clc,
	...
	reg = readl_relaxed(timer->base + KONA_GPTIMER_STCS_OFFSET);
	reg &= ~KONA_GPTIMER_STCS_TIMER_MATCH_MASK;
	reg |= (1 << (KONA_GPTIMER_STCS_TIMER_MATCH_SHIFT + channel->id));
	reg |= (1 << (KONA_GPTIMER_STCS_COMPARE_ENABLE_SHIFT + channel->id));
	writel_relaxed(reg, timer->base + KONA_GPTIMER_STCS_OFFSET);
			    ...
			    unsigned int cpu)
{
	...
	// TODO - bumping up the rating causes a system freeze after a while.
	// This needs to be investigated - it's likely that clockevents are broken.
	// channel->clockevent.rating = 250;
...
static int kona_timer_cpu_start(unsigned int cpu)
{
	...
	pr_info("kona-timer: cpu start for cpu %d\n", cpu);

	kona_timer_clockevents_init(timer, channel, cpu);
	pr_info("kona-timer: requesting irq\n");
	if (request_irq(channel->irq, kona_timer_interrupt,
			IRQF_TIMER, "Kona Timer Tick",
			channel)) {
		pr_err("kona-timer: request_irq() failed\n");
	}
	//pr_info("kona-timer: setting next event\n");
	kona_timer_set_next_event((timer->rate / HZ), &channel->clockevent);
...
static int __init kona_timer_init(struct device_node *node)
{
	...
	is_gptimer = of_property_read_bool(node, "brcm,gp-timer");
	if (is_gptimer) {
		for (i = 0; i < timer->num_channels; i++)
			kona_timer_disable_and_clear(timer->base, 0);
		kona_timer_clocksource_init(timer);
	}

	is_localtimer = of_property_read_bool(node, "brcm,local-timer");
	if (is_localtimer) {
		pr_info("kona-timer: timer %d is local timer\n", num_timers);

		/* Initialize timer channels */
		for (i = 0; i < timer->num_channels; i++) {
			pr_info("kona-timer: initializing timer channel %d\n", i);
			timer->channels[i].id = i;
			timer->channels[i].timer_id = num_timers;
			timer->channels[i].irq = irq_of_parse_and_map(node, i);
		};

		pr_info("kona-timer: done, installing hotplug callback\n");
	...
	pr_info("kona-timer: initialized timer %d, %d channels, rate %d\n",
