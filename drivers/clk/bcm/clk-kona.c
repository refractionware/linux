...
static int pll_reset(struct ccu_data *ccu, struct pll_clk_data *pll)
{
...
compute_pll_rate(struct kona_clk *clk, u32 pdiv, u32 ndiv, u32 nfrac,
		 u32 frac_div)
{
	unsigned long xtal_rate = clk_hw_get_rate(clk_hw_get_parent(&clk->hw));
	u64 rate;
...
{
	struct pll_clk_data *pll = clk->u.pll;
	unsigned long xtal_rate = clk_hw_get_rate(clk_hw_get_parent(&clk->hw));
...
static int kona_pll_clk_is_enabled(struct clk_hw *hw)
{
	struct kona_clk *bcm_clk = to_kona_clk(hw);
	struct pll_clk_data *pll = bcm_clk->u.pll;
	struct bcm_pll_pwrdwn *pwrdwn = &bcm_clk->u.pll->pwrdwn;
	struct ccu_data *ccu = bcm_clk->ccu;
	u32 reg_val;
...
static int kona_pll_clk_determine_rate(struct clk_hw *hw,
				struct clk_rate_request *req)
{
	struct kona_clk *bcm_clk = to_kona_clk(hw);

	req->rate = compute_pll_divs(bcm_clk, (u32)req->rate, NULL, NULL, NULL);

	return 0;
}

const struct clk_ops kona_pll_clk_ops = {
	...
	.determine_rate = kona_pll_clk_determine_rate,
};
